...
def compare_saved_models(
    ...
    # Load baseline models dynamically (all saved regressors, excluding classifier/stacking).
    model_names = []
    for path in sorted(baseline_path.glob("*_model.pkl")):
        stem = path.stem
        if not stem.endswith("_model"):
            continue
        name = stem[: -len("_model")]
        if name in {"category_classifier", "stacking"}:
            continue
        model_names.append(name)

    if not model_names:
        print(f"Error: No baseline regressors found in {baseline_path}")
        return None
    ...
    for name in model_names:
        ...
        if not path.exists():
            path = new_path / f"{name}_model.pkl"
        try:
            models_b[name] = joblib.load(path)
        except (OSError, FileNotFoundError):
            print(f"Warning: Could not load new model {name} from {path}. Skipping.")
    ...
    weights_a = {name: weight for name, weight in weights_a.items() if name in models_a}
    if not weights_a:
        weights_a = {name: 1.0 / len(models_a) for name in models_a}

    weights_b = {name: weight for name, weight in weights_a.items() if name in models_b}
    if not weights_b:
        weights_b = {name: 1.0 / len(models_b) for name in models_b}
