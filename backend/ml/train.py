...
# TabICL v2 package names are not stable across releases.
try:
    from tabicl import TabICLClassifier, TabICLRegressor
except Exception:
    try:
        from tabicl_v2 import TabICLClassifier, TabICLRegressor
    except Exception:
        try:
            from tabiclv2 import TabICLClassifier, TabICLRegressor
        except Exception:
...
def _build_tabicl_regressor() -> object:
    """Build a conservative TabICL regressor config for CPU-only small-tabular runs."""
    if TabICLRegressor is None:
        raise RuntimeError("TabICLRegressor is not available")

    checkpoint_path = os.getenv("TABICL_REGRESSOR_CHECKPOINT")
...
def train_dedicated_category_classifier(
    ...
    sample_weight: np.ndarray | None = None,
    include_experimental_models: bool = False,
    ...
    diagnostics = {
        ...
        "xgb_classifier_available": XGBClassifier is not None,
        "tabicl_v2_classifier_available": TabICLClassifier is not None,
        "tabicl_v2_classifier_enabled": False,
        "tabicl_v2_classifier_disabled_reason": "disabled_due_runtime_instability",
        "experimental_models_enabled": bool(include_experimental_models),
    ...
    if include_experimental_models:
        classifier_specs["extra_trees_classifier"] = {
        ...
        classifier_specs["hist_gb_classifier"] = {
...
def train_models(
    ...
    enable_experimental_tabular: bool = False,
) -> tuple[dict, dict]:
    """
    ...
    Args:
        ...
        enable_experimental_tabular: If True, include additional tabular candidates
            (ExtraTrees, HistGB, CatBoost regressor, and optional TabICL v2 if available)
    ...
    for strategy_name, thresholds in threshold_candidates.items():
        ...
        for fs_seed in category_feature_selection_seeds:
            ...
            model, model_meta = train_dedicated_category_classifier(
                ...
                include_experimental_models=enable_experimental_tabular,
            )
            best_metrics = (
    ...
    if enable_experimental_tabular:
        ...
        if CatBoostRegressor is not None:
            ...
            print("Skipping CatBoost regressor: package not installed")

        if TabICLRegressor is not None:
            model_specs["tabicl_v2"] = (
                "TabICL v2 Regressor (foundation tabular)",
                lambda: _build_tabicl_regressor(),
            )
        else:
            print("Skipping TabICL v2 regressor: package not installed")
    ...
    for model_key, (label, model_factory) in model_specs.items():
        ...
        try:
            ...
            print(f"  Warning: could not train {model_key}: {exc}")

    if not models:
        raise RuntimeError("No regression models could be trained successfully.")
    ...
    metrics.update(
        {
            ...
            "experimental_tabular": {
                "enabled": bool(enable_experimental_tabular),
                "tabicl_v2_available": TabICLRegressor is not None or TabICLClassifier is not None,
            },
...
def save_models(models: dict, metrics: dict | None = None):
    ...
    if metrics:
        ...
        meta = {
            ...
            "experimental_tabular": metrics.get("experimental_tabular"),
...
def load_models(load_stacking: bool = False) -> tuple[dict | None, dict | None]:
    ...
    for path in sorted(models_dir.glob("*_model.pkl")):
        ...
        if not name.endswith("_model"):
            continue
        model_name = name[: -len("_model")]
        if model_name == "stacking" and not load_stacking:
            continue
        try:
    ...
    # Backward-compatible fallback in case stacking artifact naming changes.
    if load_stacking and "stacking" not in models:
...
if __name__ == "__main__":
    ...
    regression_metric_keys = [
        ...
        if isinstance(payload, dict) and "mae_cv" in payload and "mae_std" in payload
    ]
    for model_name in sorted(regression_metric_keys):
    ...
    print("\nModel Configuration:")
    if regression_metric_keys:
        first_key = regression_metric_keys[0]
        print(f"   Features used:              {metrics[first_key]['features']}")
        print(f"   Training samples:           {metrics[first_key]['samples']}")
