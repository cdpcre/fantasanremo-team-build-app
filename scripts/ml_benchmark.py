...
def main() -> int:
    ...
    parser.add_argument(
        "--experimental-tabular",
        action="store_true",
        help=(
            "Enable additional tabular candidates (ExtraTrees/HistGB/CatBoost and optional "
            "TabICL v2 when available)"
        ),
    )
    args = parser.parse_args()

    print("Running ML benchmark...")
    models, metrics = train_models(enable_experimental_tabular=args.experimental_tabular)
